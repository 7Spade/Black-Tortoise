...
export class PermissionsModule implements IAppModule, OnInit, OnDestroy {
  ...
  initialize(eventBus: IModuleEventBus): void {
    ...
    this.subscriptions.add(
      ModuleEventHelper.onWorkspaceSwitched(eventBus, (event) => {
        console.log(`[PermissionsModule] Workspace switched:`, event);
      })
    );
    
    // Publish initialization event
    ModuleEventHelper.publishModuleInitialized(eventBus, this.id);
    console.log(`[PermissionsModule] Initialized`);
  }
  
  activate(): void {
    console.log(`[PermissionsModule] Activated`);
  }
  
  deactivate(): void {
    console.log(`[PermissionsModule] Deactivated`);
  }
  
  destroy(): void {
    this.subscriptions.unsubscribeAll();
    console.log(`[PermissionsModule] Destroyed`);
  }
