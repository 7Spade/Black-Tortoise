...
import { SwitchWorkspaceHandler } from '../handlers/switch-workspace.handler';
  ...
  withMethods((store) => {
    ...
    return {
      ...
      switchWorkspace(workspaceId: string): void {
        const workspace = store.availableWorkspaces().find(w => w.id.getValue() === workspaceId);
        ...
        const previousWorkspace = store.currentWorkspace();
        const previousWorkspaceId = previousWorkspace ? previousWorkspace.id.getValue() : null;
        ...
        if (workspace.ownerType === 'organization') {
          const org = organizationStore.findOrganizationById(workspace.ownerId.getValue());
          if (org) {
      ...
      findWorkspaceById(workspaceId: string): Workspace | undefined {
        return store.availableWorkspaces().find(w => w.id.getValue() === workspaceId);
      },
